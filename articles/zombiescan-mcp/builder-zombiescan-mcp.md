...
*Subtitle: An agent front door for a local AWS cost scanner, where every tool returns the total already computed*
...
- Rounding each finding before adding costs two cents against the exact sum on this account
- The ECR estimate is 2.1x the billed figure: $6.40 against $2.99 projected from 19.96 GB-Month, because shared layers are counted once per image
