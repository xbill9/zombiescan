...
*Subtitle: A local-first CLI that scans an AWS account for unused resources, prices each one from the AWS Price List API, and writes the cleanup*
...
- `unused-ami` reads instances only; launch templates and Auto Scaling groups stay outside its view
- Costs are on-demand list prices, excluding savings plans, reserved capacity, private pricing and credits
