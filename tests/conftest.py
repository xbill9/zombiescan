...
class FakePaginator:
    def __init__(self, pages: list[dict[str, Any]], operation: str, client: FakeClient) -> None:
        self._pages = pages
        self._operation = operation
        self._client = client

    def paginate(self, **kwargs: Any) -> list[dict[str, Any]]:
        self._client.calls[self._operation] = kwargs
        return self._pages


class FakeClient:
    """Stands in for a boto3 EC2 client.

    ``pages`` maps a paginated operation to the pages it yields; ``direct``
    maps a non-paginated operation to its single response. Every call is
    recorded in ``calls`` so tests can assert on server-side filtering.
    """
    ...
    def describe_addresses(self, **kwargs: Any) -> dict[str, Any]:
        self.calls["describe_addresses"] = kwargs
        return self._direct.get("describe_addresses", {"Addresses": []})
